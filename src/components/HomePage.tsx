...
export default function HomePage() {
  return (
    <main className="flex flex-col items-center bg-white text-slate-800 font-sans">
      ...
      <header className="w-full bg-white py-2 shadow-sm z-50">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between flex-wrap">
          ...
          <nav className="sc-topnav flex justify-end items-center flex-wrap gap-x-6 gap-y-2 text-base md:text-lg font-semibold tracking-wide">
            {["Home", "About", "Solutions", "Blog", "Reviews", "FAQ", "Contact"].map((item) => (
              <a
                key={item}
      ...
      {/* === BLOG SECTION === */}
      <section id="blog" className="w-full py-16 px-6 bg-gray-50 text-center">
        <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-8">Blog</h2>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 max-w-6xl mx-auto">
          {[
            { name: "All Posts", href: "/blog", summary: "Browse every RoofPro article in one place." },
            ...
            <Link
              key={item.href}
              href={item.href}
              className="block rounded-xl border border-slate-200 bg-white p-5 shadow-sm hover:shadow-md transition text-left"
            >
              <h3 className="text-lg font-semibold text-slate-900">{item.name}</h3>
              <p className="text-slate-700 mt-2 text-sm">{item.summary}</p>
              <span className="mt-3 inline-block text-blue-800 font-semibold">Read more →</span>
            </Link>
          ))}
        </div>
      </section>
