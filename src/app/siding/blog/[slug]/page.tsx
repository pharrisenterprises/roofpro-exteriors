...
export async function generateMetadata(
  ...
  const canonical = `${SITE_URL}/${data?.service?.slug ?? "siding"}/blog/${slug}`;
...
export default async function SidingPostPage(
  { params }: { params: { slug: string } }
) {
  const { slug } = params;
  ...
  const canonical = `${SITE_URL}/${post.service?.slug ?? "siding"}/blog/${post.slug.current}`;
