...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = data?.seo?.title ?? "Exterior Repairs | RoofPro Exteriors";
  const description =
    data?.seo?.description ?? "Exterior repairs, fascia, soffit, and carpentry fixes in Richmond, VA.";
  const ogSrc = data?.seo?.ogImage ?? data?.heroImage;
  const images = ogSrc ? [{ url: urlFor(ogSrc).width(1200).height(630).url() }] : undefined;
  const canonical = `${SITE_URL}/exterior-repairs`;
  return {
    title,
    description,
  ...
  if (!data) {
    return (
      <main className="mx-auto max-w-4xl px-4 py-12">
        <h1 className="text-3xl font-bold">Exterior Repairs</h1>
